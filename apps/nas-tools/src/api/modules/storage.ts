...
export async function getStorageBackendsApi() {
  return requestClient.post<{ count: number; items: StorageApi.StorageBackend[] }>('/api/storage/list', {});
}

/** 获取单个存储后端 */
export async function getStorageBackendApi(sid: number) {
  return requestClient.post<{ data: StorageApi.StorageBackend }>('/api/storage/get', { sid });
}
...
/** 测试存储后端连接 */
export async function testStorageBackendApi(data: { type: string; config: string }) {
  return requestClient.post<{ success: boolean; msg?: string }>('/api/storage/test', data);
}

/** 获取支持的存储类型 */
export async function getStorageTypesApi() {
  return requestClient.post<{ items: StorageTypeSchema[] }>('/api/storage/types', {});
}
