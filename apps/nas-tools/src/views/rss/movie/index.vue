...
import {
  ...
  updateRssApi,
  removeRssMediaApi,
  getDefaultRssSettingApi,
  saveDefaultRssSettingApi,
  getRssDetailApi,
...
async function fetchData() {
  loading.value = true;
  try {
    const res: any = await getMovieRssApi();
    const list = Array.isArray(res) ? res : (res?.data || []);
    rssStore.setMovieSubscriptions(list);
  } finally {
    loading.value = false;
  }
}
...
async function handleDetailRefresh() {
  try {
    await refreshRssApi('MOV', String(detailItem.value.id));
...
async function handleEdit(item: any) {
  let detail: any = {};
  try {
    const res: any = await getRssDetailApi(item.id, 'MOV');
...
async function handleRefresh(item: any) {
  try {
    await refreshRssApi('MOV', String(item.id));
    notification.success({ content: '已触发刷新' });
  } catch (err: any) {
    notification.error({ content: '刷新失败', description: err?.message || '' });
  }
...
async function selectAddMedia(media: any) {
  ...
  let defaults: any = {};
  try {
    const res: any = await getDefaultRssSettingApi('MOV');
    defaults = res?.data || {};
  } catch {
    // ignore
  }
...
async function confirmAdd() {
  try {
    await addRssApi({
      ...
      filter_restype: addForm.value.filter_restype || undefined,
      filter_pix: addForm.value.filter_pix || undefined,
      filter_team: addForm.value.filter_team || undefined,
      filter_rule: addForm.value.filter_rule || undefined,
      filter_include: addForm.value.filter_include || undefined,
      filter_exclude: addForm.value.filter_exclude || undefined,
...
async function confirmSetting() {
  try {
    await saveDefaultRssSettingApi('MOV', {
...
// ---------- 站点全选 ----------
function toggleAllSites(target: 'addRss' | 'addSearch' | 'settingRss' | 'settingSearch', checked: boolean) {
  if (target === 'addRss') addForm.value.rss_sites = checked ? rssSites.value.map(s => s.value) : [];
  if (target === 'addSearch') addForm.value.search_sites = checked ? searchSites.value.map(s => s.value) : [];
  if (target === 'settingRss') settingForm.value.rss_sites = checked ? rssSites.value.map(s => s.value) : [];
  if (target === 'settingSearch') settingForm.value.search_sites = checked ? searchSites.value.map(s => s.value) : [];
}

function isAllSelected(target: 'addRss' | 'addSearch' | 'settingRss' | 'settingSearch') {
  let arr: string[] = [];
  let all: string[] = [];
  if (target === 'addRss') { arr = addForm.value.rss_sites; all = rssSites.value.map(s => s.value); }
  if (target === 'addSearch') { arr = addForm.value.search_sites; all = searchSites.value.map(s => s.value); }
  if (target === 'settingRss') { arr = settingForm.value.rss_sites; all = rssSites.value.map(s => s.value); }
...
<template>
  <div class="p-4">
    ...
    <NSpin :show="loading">
      <div v-if="rssStore.movieSubscriptions.length > 0" class="grid-rss-card">
        <div
          v-for="item in rssStore.movieSubscriptions"
          :key="item.id"
          class="rss-card overflow-hidden rounded cursor-pointer tbl-card tbl-card-hover"
    ...
    >
      <div v-if="addStep === 'search'">
        <NSpace vertical>
          ...
          <div v-if="addSearchResults.length > 0" class="grid-rss-card mt-2" style="max-height: 50vh; overflow-y: auto;">
            <div
        ...
        <NForm label-placement="left" label-width="90" size="small">
          ...
          <div class="grid grid-cols-3 gap-3">
            <NFormItem label="模糊匹配">
              <NCheckbox v-model:checked="addForm.fuzzy_match">开启</NCheckbox>
            </NFormItem>
            <NFormItem label="洗版">
              <NCheckbox v-model:checked="addForm.over_edition">开启</NCheckbox>
            </NFormItem>
            <NFormItem />
          </div>
          <div class="grid grid-cols-3 gap-3">
          ...
          <NFormItem label="订阅站点">
            <NCard size="small" :bordered="true" class="site-card">
              <div class="flex items-center gap-2 mb-2 pb-2 border-b border-gray-100">
                <NCheckbox :checked="isAllSelected('addRss')" @update:checked="v => toggleAllSites('addRss', v)">全选</NCheckbox>
              </div>
              <div class="flex flex-wrap gap-x-4 gap-y-2">
                <NCheckbox v-for="site in rssSites" :key="site.value" :value="site.value" :checked="addForm.rss_sites.includes(site.value)" @update:checked="(v: boolean) => { const set = new Set(addForm.rss_sites); if (v) set.add(site.value); else set.delete(site.value); addForm.rss_sites = Array.from(set); }">
                  {{ site.label }}
                </NCheckbox>
              </div>
            </NCard>
      ...
      <NForm label-placement="left" label-width="100">
        ...
        <NFormItem label="订阅站点">
          <div class="flex items-center gap-2 mb-1">
            <NCheckbox :checked="isAllSelected('settingRss')" @update:checked="v => toggleAllSites('settingRss', v)">全选</NCheckbox>
          </div>
          <div class="flex flex-wrap gap-2">
            <NCheckbox v-for="site in rssSites" :key="site.value" :value="site.value" :checked="settingForm.rss_sites.includes(site.value)" @update:checked="(v: boolean) => { const set = new Set(settingForm.rss_sites); if (v) set.add(site.value); else set.delete(site.value); settingForm.rss_sites = Array.from(set); }">
              {{ site.label }}
            </NCheckbox>
          </div>
        </NFormItem>
