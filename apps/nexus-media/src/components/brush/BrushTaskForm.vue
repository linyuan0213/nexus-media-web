    ...
    <div class="form-section">
      ...
      <div class="form-grid">
        <NFormItem label="任务名称" path="brushtask_name" required>
          <NInput v-model:value="form.brushtask_name" placeholder="请输入任务名称" />
        </NFormItem>
        <NFormItem path="brushtask_rule_id" required>
          <template #label>
            <component :is="() => labelWithHelp('规则模板', '选择已保存的刷流规则模板，选种/删种/停种规则从模板读取')" />
          </template>
          <NSelect
        ...
        <NFormItem label="执行周期" path="brushtask_interval" required>
          <template #label>
            <component :is="() => labelWithHelp('执行周期', '检查站点RSS更新的周期，为了减小站点压力，建议不小于5分钟，支持两种配置方式：1、间隔时间（分钟），如 10；2、5位cron表达式,如：*/30 * * * *（对应：分 时 日 月 星期）')" />
          </template>
          <NInput v-model:value="form.brushtask_interval" placeholder="10 或 */10 * * * *" />
        </NFormItem>
        <NFormItem label="下载器" path="brushtask_downloader" required>
          <template #label>
            <component :is="() => labelWithHelp('下载器', '选择刷流任务使用的下载器，在设置-下载器中添加，如需识别转移到媒体库，所选下载器也需启用监控功能')" />
          </template>
          <NSelect
        ...
        <NFormItem path="brushtask_totalsize">
          <template #label>
            <component :is="() => labelWithHelp('保种体积(GB)', '当前刷流任务下载做种超过设定的体积大小时不再新增下载')" />
          </template>
          <NInput v-model:value="form.brushtask_totalsize" placeholder="留空不限制" />
        </NFormItem>
        <NFormItem path="brushtask_label">
          <template #label>
            <component :is="() => labelWithHelp('标签', '用户在下载器中标记该任务的种子，多个标签使用,分隔')" />
          </template>
          <NInput v-model:value="form.brushtask_label" placeholder="多个标签使用,分隔" />
        </NFormItem>
        ...
        <NFormItem path="brushtask_time_range">
          <template #label>
            <component :is="() => labelWithHelp('开启时间段', '为该刷新任务设置独立的刷流时间段格式为HH:MM-HH:MM，支持多个时间段按逗号分隔，为空不开启')" />
          </template>
          <NInput v-model:value="form.brushtask_time_range" placeholder="如: 08:00-22:00" />
        </NFormItem>
      </div>
    </div>
    ...
    <div class="form-section">
      ...
      <div class="form-grid">
        <NFormItem path="brushtask_rssurl" class="form-col-span-3">
          <template #label>
            <component :is="() => labelWithHelp('RSS地址', '刷流优先使用此处填入的站点RSS，若为空则使用站点配置RSS地址')" />
          </template>
          <NInput v-model:value="form.brushtask_rssurl" placeholder="站点RSS订阅URL，若为空则使用站点配置RSS地址" />
        </NFormItem>
      </div>
    </div>
    ...
    <div class="form-section form-section-inline">
      <div class="form-grid-inline">
        <NFormItem path="brushtask_sendmessage">
          <template #label>
            <component :is="() => labelWithHelp('消息推送', '开启后会将当前任务的情况进行推送')" />
          </template>
          <NSwitch v-model:value="form.brushtask_sendmessage" />
        </NFormItem>
        <NFormItem path="brushtask_transfer">
          <template #label>
            <component :is="() => labelWithHelp('转移到媒体库', '开启后刷流的下载也会进行转移并识别重命名到媒体库')" />
          </template>
