<template>
  <div class="p-5" style="background: hsl(var(--background))">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-5">
      <div class="stat-card">
        <div class="stat-value" style="color: hsl(var(--primary))">{{ stats.total_keys }}</div>
        <div class="stat-label">总 Key 数</div>
      </div>
      <div class="stat-card">
        <div class="stat-value" style="color: hsl(var(--success))">{{ stats.active_keys }}</div>
        <div class="stat-label">活跃 Key</div>
      </div>
      <div class="stat-card">
        <div class="stat-value" style="color: hsl(var(--warning))">{{ stats.total_requests }}</div>
        <div class="stat-label">总请求数</div>
      </div>
      <div class="stat-card">
        <div class="stat-value" style="color: hsl(var(--info))">{{ stats.today_requests }}</div>
    ...
    <NSpin :show="loading" class="mt-5">
      <!-- API Key 卡片网格 -->
      <div v-if="keys.length > 0" class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        <div
          v-for="item in keys"
          :key="item.id"
          class="apikey-card"
          :class="{ 'apikey-card--expired': item.is_expired, 'apikey-card--disabled': item.status !== 1 }"
        >
          <!-- 顶部：名称 + 状态 -->
          <div class="apikey-card-header">
            <div class="apikey-name">{{ item.name }}</div>
            <div class="apikey-status">
              <span v-if="item.is_expired" class="status-badge status-badge--expired">已过期</span>
              <span v-else-if="item.status === 1" class="status-badge status-badge--active">启用</span>
              <span v-else class="status-badge status-badge--disabled">禁用</span>
            </div>
          </div>

          <!-- Key 值 -->
          <div class="apikey-key-section">
            <IconifyIcon icon="lucide:key" class="size-4" style="color: hsl(var(--muted-foreground))" />
          ...
          <div class="apikey-info">
            ...
            <div class="apikey-info-item">
              ...
              <span>{{ item.expires_at ? formatTime(item.expires_at) : '永不过期' }}</span>
            </div>
            <div class="apikey-info-item">
    ...
    >
      ...
      >
        <NFormItem label="名称" path="name">
          <NInput v-model:value="createForm.name" placeholder="用于标识此 Key 的用途" />
        </NFormItem>
    ...
    >
      ...
      <div class="flex items-center gap-2 p-3 rounded" style="background: hsl(var(--muted))">
    ...
    >
      确定要删除 API Key <strong>{{ deleteTarget?.name }}</strong> 吗？此操作不可恢复。
    </NModal>
  </div>
</template>
...
const stats = ref({ total_keys: 0, active_keys: 0, total_requests: 0, today_requests: 0 });
const keys = ref<any[]>([]);
...
const createForm = ref({ name: '', expires_days: null as number | null, description: '' });
...
const logColumns = [
  { title: 'Request ID', key: 'request_id', minWidth: 200, ellipsis: { tooltip: true } },
  { title: '名称', key: 'request_name', width: 120, render(row: any) { return row.request_name || '-'; } },
...
function copyKey() {
  navigator.clipboard.writeText(createdKey.value).then(() => message.success('已复制'));
}

function openEdit(row: any) {
  editForm.value = { id: row.id, name: row.name, status: row.status, description: row.description || '' };
...
async function handleUpdate() {
  try {
    ...
    await updateAPIKeyApi(editForm.value.id, { name: editForm.value.name, status: editForm.value.status, description: editForm.value.description });
...
async function fetchLogs() {
  ...
  try {
    const res: any = await listAPIKeyLogsApi(selectedKeyId.value, logPage.value, logPageSize.value);
...
function handlePageChange(p: number) { page.value = p; fetchKeys(); }
function handleLogPageChange(p: number) { logPage.value = p; fetchLogs(); }

onMounted(() => { fetchKeys(); fetchStats(); });
</script>
