...
async function confirmAdd() {
  try {
    await addSubscriptionApi({
      ...
      total_ep: addForm.value.total_ep
        ? Number(addForm.value.total_ep)
        : undefined,
      current_ep: addForm.value.current_ep
        ? Number(addForm.value.current_ep)
        : undefined,
...
function toggleAllSites(target: string, checked: boolean) {
  ...
  if (target === 'addSubscription')
    addForm.value.rss_sites = checked ? [...all] : [];
  if (target === 'addSearch')
    addForm.value.search_sites = checked ? [...allSearch] : [];
      ...
      <div v-else>
        ...
        <NForm label-placement="left" label-width="90" size="small">
          ...
          <div class="grid grid-cols-3 gap-3">
            <NFormItem label="季">
              <NSelect
                v-model:value="addForm.season"
                :options="seasonOptions"
              />
            </NFormItem>
            <NFormItem label="总集数">
              <NInput v-model:value="addForm.total_ep" placeholder="可留空" />
            </NFormItem>
            <NFormItem label="开始集数">
              <NInput
                v-model:value="addForm.current_ep"
                placeholder="开始订阅集数"
              />
            </NFormItem>
          </div>
          <div class="grid grid-cols-3 gap-3">
            <NFormItem label="模糊匹配">
              <NCheckbox v-model:checked="addForm.fuzzy_match">
                开启
              </NCheckbox>
            </NFormItem>
            <NFormItem label="洗版">
              <NCheckbox v-model:checked="addForm.over_edition">
                开启
              </NCheckbox>
            </NFormItem>
            <NFormItem />
